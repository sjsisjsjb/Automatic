...
function generateUniqueNominal(input) {
    const today = new Date().toISOString().split('T')[0];
    const filePath = path.join(__dirname, `./database/nominal/${today}.json`);
    let usedNumbers = [];
    if (fs.existsSync(filePath)) {
        const data = fs.readFileSync(filePath, 'utf8');
        usedNumbers = JSON.parse(data);
    }

    let lowerLimit = input;
    let upperLimit = lowerLimit + 99;

    let randomNumber = lowerLimit;

    while (usedNumbers.includes(randomNumber) && randomNumber <= upperLimit) {
        randomNumber++;
    }
    if (randomNumber > upperLimit) {
        throw new Error("Semua angka dalam rentang telah digunakan!");
    }

    usedNumbers.push(randomNumber);
    fs.writeFileSync(filePath, JSON.stringify(usedNumbers, null, 2));

    return randomNumber;
}

function get30DaysFromNowWIB() {
  const now = new Date();
...
module.exports = { unixTimestampSeconds, generateMessageTag, processTime, webApi, getRandom, getBuffer, fetchJson, runtime, clockString, sleep, isUrl, getTime, formatDate, tanggal, formatp, jsonformat, reSize, toHD, logic, generateProfilePicture, bytesToSize, checkBandwidth, getSizeMedia, parseMention, getGroupAdmins, readFileTxt, readFileJson, getHashedPassword, generateAuthToken, cekMenfes, generateToken, batasiTeks, randomText, isEmoji, getTypeUrlMedia, pickRandom, getAllHTML, toIDR, capital, generateUniqueNominal, get30DaysFromNowWIB};
